...
// ↑ Mantenemos tus mismos iconos y estilo

// Ahora cada menú puede tener un 'perm' (slug de permisos.modulo).
// Si 'perm' no está, se controla solo por roles (como antes).
const Menus = [
  { title: 'Dashboard', src: Chart, path: '/dashboard', roles: ['admin', 'operador', 'consulta'] }, // sin perm
  { title: 'Zonas', src: Folder, path: '/zonas', roles: ['admin', 'operador', 'consulta'], perm: 'zonas' },
  { title: 'Familias', src: Folder, path: '/familias', roles: ['admin', 'operador', 'consulta'], perm: 'familias' },
  ...
  { title: 'Ventas', src: Folder, path: '/ventas', roles: ['admin', 'operador', 'consulta'], perm: 'venta_cajas' },
  { title: 'Benefactores', src: User, path: '/benefactores', roles: ['admin', 'operador', 'consulta'], perm: 'familia_benefactor' },
  ...
  { title: 'Configuración', src: Setting, path: '/configuracion', roles: ['admin'], perm: 'configuracion' } // admin + (opcional) perm
];
...
export default function Sidebar() {
  ...
  // ¿Tiene el permiso 'slug'? -> admin siempre puede
  const can = (slug) => {
    if (!slug) return true;                // items sin permiso requerido
    if (user?.rol === 'admin') return true;
    const granted = (user?.permisos || []).map(p => p.modulo ?? p.nombre);
  ...
  // Filtrar por rol Y permiso (si aplica)
